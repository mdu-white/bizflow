...
export class InvoicesService {
  ...
  private validateStatusTransition(
    ...
    if (currentStatus === "DRAFT") {
      ...
      throw new BadRequestException(
        `Invoice in DRAFT status cannot transition to ${nextStatus}`
      );
    }
    ...
    if (currentStatus === "SENT") {
      ...
      throw new BadRequestException(
        `Invoice in SENT status cannot transition to ${nextStatus}`
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        "Partially paid invoices cannot be manually updated. Record additional payments instead."
      );
    }

    if (currentStatus === "PAID") {
      throw new BadRequestException(
        "Paid invoices cannot be manually updated."
      );
    }

    if (currentStatus === "VOID") {
      throw new BadRequestException(
        "Void invoices cannot be updated."
      );
    }
  }
  ...
  async addPayment(
    ...
    if (
      ...
      throw new BadRequestException(
        `Payment cannot exceed outstanding balance of ${outstandingCents} cents.`
      );
    }
    ...
    const totals =
      await this.prisma.invoicePayment.aggregate(
        {
          where: {
            invoiceId
          },
