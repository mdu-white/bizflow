...
export class DirectorLoansService {
  ...
  async create(userId: string, dto: CreateDirectorLoanDto) {
    const organizationId =
      await this.organizationContext.getOrganizationId(userId);

    const account = await this.prisma.directorLoanAccount.create({
      data: {
        organizationId,
  ...
  async findOne(userId: string, accountId: string) {
    ...
    const account = await this.prisma.directorLoanAccount.findFirst({
      where: {
        id: accountId,
        organizationId,
        deletedAt: null,
    ...
    if (!account) {
      throw new NotFoundException("Director loan account not found");
    }

    return account;
  }

  private ensureAccountIsEditable(status: DirectorLoanStatus) {
    if (status === DirectorLoanStatus.SETTLED) {
      throw new BadRequestException(
        "Settled director loan accounts cannot be modified",
      );
    }
  }

  async update(userId: string, accountId: string, dto: UpdateDirectorLoanDto) {
    const account = await this.findOne(userId, accountId);

    this.ensureAccountIsEditable(account.status);

    const updated = await this.prisma.directorLoanAccount.update({
      where: {
        id: account.id,
      },
      data: dto,
  ...
  async remove(userId: string, accountId: string) {
    ...
    const deleted = await this.prisma.directorLoanAccount.update({
      where: {
        id: account.id,
      },
      data: {
  ...
  private calculateStatus(balance: number): DirectorLoanStatus {
    if (balance === 0) {
  ...
  async createTransaction(
    ...
    const transaction = await this.prisma.directorLoanTransaction.create({
      data: {
    ...
    await this.prisma.directorLoanAccount.update({
      ...
      data: {
        ...
        status: this.calculateStatus(newBalance),
      },
    });
